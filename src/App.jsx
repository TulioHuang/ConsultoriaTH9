...
function gerarMsgCobranca(a) {
  const plano = a.plano?.split(" - ")[0]||"Mensal";
  const preco = a.plano?.split(" - ")[1]||"";
  return `Olá, ${a.nome}! 👋\n\nSua mensalidade do plano *${plano}* vence em breve.\n\n📅 Vencimento: ${fmtDateInput(a.vencimento)}\n💰 Valor: *${preco}*\n\nPara renovar é só me chamar! 🔥\n— Tulio | Consultoria TH9`;
}
...
const DEMO_HIST = {
  a1: {
    dietas: [{id:"d1",data:now(),kcal:2200,prot:145,carbo:220,gord:65,obs:"Déficit 400kcal. Base em alimentos práticos.",refeicoes:"Café: ovos + pão integral\nAlmoço: frango + arroz + legumes\nJantar: carne + batata-doce"}],
    ...
    checkins: [{id:"c1",data:now(),nota:"5 de 6 treinos. Sono melhorou. Foco total!",peso:82.5}],
  },
  a2: {
...
.app{max-width:480px;margin:0 auto;min-height:100vh;background:${B}}
.hdr{background:${D};border-bottom:1px solid ${G}22;padding:16px 18px 0;position:sticky;top:0;z-index:100}
.hrow{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}
.logo{display:flex;align-items:center;gap:9px}
.lbox{width:34px;height:34px;background:${G};border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:12px;color:${B}}
.ltxt{font-size:11px;font-weight:800;letter-spacing:2px;color:${W};text-transform:uppercase}
.lsub{font-size:8px;color:${G};font-weight:600;letter-spacing:1px}
.live{display:flex;align-items:center;gap:4px;font-size:8px;color:#22c55e;font-weight:700;letter-spacing:.5px}
.ldot{width:5px;height:5px;border-radius:50%;background:#22c55e;animation:pulse 1.5s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}
.nav{display:flex}
.ntab{flex:1;padding:10px 3px;text-align:center;font-size:8.5px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:#444;cursor:pointer;border-bottom:2px solid transparent;transition:all .2s;background:none;border-top:none;border-left:none;border-right:none;font-family:'Montserrat',sans-serif}
.ntab.active{color:${G};border-bottom-color:${G}}
...
.card{background:${D};border:1px solid ${GR};border-radius:12px;padding:14px;margin-bottom:10px;transition:border-color .2s}
...
.cs{font-size:10px;color:#666;margin-top:2px;font-weight:500}
.div{height:1px;background:${GR};margin:8px 0}
.inf{display:flex;align-items:flex-start;gap:7px;margin-top:6px}
.il{font-size:9px;color:#444;font-weight:700;letter-spacing:.3px;text-transform:uppercase;min-width:66px;padding-top:1px}
...
.fab{position:fixed;bottom:22px;right:22px;width:48px;height:48px;border-radius:50%;background:${G};color:${B};border:none;font-size:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px ${G}44;z-index:150;transition:all .2s}
...
.modal{background:${D};border-top:1px solid ${G}44;border-radius:20px 20px 0 0;padding:20px 18px 44px;width:100%;max-width:480px;max-height:92vh;overflow-y:auto;animation:su .28s ease}
...
.hero-fin{background:linear-gradient(135deg,${GR},${D});border:1px solid ${G}33;border-radius:14px;padding:18px;margin-bottom:14px;position:relative;overflow:hidden}
.hero-fin::before{content:'';position:absolute;top:-20px;right:-20px;width:100px;height:100px;border-radius:50%;background:${G}08;border:1px solid ${G}15}
.hero-lbl{font-size:8px;font-weight:700;letter-spacing:2px;color:${G};text-transform:uppercase;margin-bottom:5px;opacity:.8}
.hero-val{font-size:34px;font-weight:900;color:${W};line-height:1;margin-bottom:3px;letter-spacing:-1px}
...
.meta-card{background:${GR};border:1px solid ${LG};border-radius:11px;padding:13px;margin-bottom:13px}
.meta-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:9px}
.meta-lbl{font-size:9px;font-weight:700;color:#666;letter-spacing:.3px;text-transform:uppercase}
.meta-pct{font-size:17px;font-weight:900;color:${G}}
...
.g2{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:13px}
.kpi{background:${GR};border:1px solid ${LG};border-radius:11px;padding:13px}
...
.cc{background:${GR};border:1px solid ${LG};border-radius:11px;padding:13px;margin-bottom:13px}
.cc-title{font-size:9px;font-weight:700;color:#555;letter-spacing:.4px;text-transform:uppercase;margin-bottom:12px}
...
.alert-b{background:#ef444411;border:1px solid #ef444422;border-radius:9px;padding:10px 12px;margin-bottom:13px;display:flex;align-items:center;gap:8px}
...
.warn-b{background:#f9731611;border:1px solid #f9731622;border-radius:9px;padding:10px 12px;margin-bottom:13px;display:flex;align-items:center;gap:8px}
...
.macro-val{font-size:16px;font-weight:900}
...
.aluno-row{display:flex;align-items:center;gap:9px;padding:9px 0;border-bottom:1px solid ${LG}}
.aluno-row:last-child{border-bottom:none}
.av{width:30px;height:30px;border-radius:50%;background:${G}22;border:1px solid ${G}33;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:${G};flex-shrink:0}
.ai{flex:1;min-width:0}
.an{font-size:11px;font-weight:700;color:${W};white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.ap{font-size:9px;color:#444;font-weight:500;margin-top:1px}
`;
...
export default function TH9Sistema() {
  ...
  function abrirCadastro(a=null) {
    setForm(a||{nome:"",whatsapp:"",plano:PLANOS[0],vencimento:"",objetivo:"",checkin:"",obs:""});
    setModal("cadastro");
  }
  ...
  function salvarAluno() {
    ...
    if(form.id) {
      ...
      const novoId="a"+uid();
      setAlunos(prev=>[...prev,{...form,id:novoId}]);
      setH(novoId,()=>({dietas:[],pesos:[],checkins:[]}));
    }
    setModal(null);
  }

  function excluirAluno(id) {
    if(confirm("Excluir aluno?")){ setAlunos(prev=>prev.filter(a=>a.id!==id)); setModal(null); }
  }

  function abrirHistorico(a) {
    setSelected(a);
    setDietaForm({kcal:"",prot:"",carbo:"",gord:"",obs:"",refeicoes:""});
    setNovoHist({peso:"",pesObs:"",ciNota:"",ciPeso:""});
    setExpandHist(null);
    setModal("historico");
  }

  function salvarDieta() {
    if(!dietaForm.kcal) return;
    setH(selected.id, h=>({...h,dietas:[{id:uid(),data:now(),...dietaForm},...(h.dietas||[])]}));
    setDietaForm({kcal:"",prot:"",carbo:"",gord:"",obs:"",refeicoes:""});
  }

  function salvarPeso() {
    if(!novoHist.peso) return;
    setH(selected.id, h=>({...h,pesos:[{id:uid(),data:now(),val:parseFloat(novoHist.peso),obs:novoHist.pesObs},...(h.pesos||[])]}));
    setNovoHist(n=>({...n,peso:"",pesObs:""}));
  }

  function salvarCheckin() {
    if(!novoHist.ciNota) return;
    setH(selected.id, h=>({...h,checkins:[{id:uid(),data:now(),nota:novoHist.ciNota,peso:novoHist.ciPeso},...(h.checkins||[])]}));
    setNovoHist(n=>({...n,ciNota:"",ciPeso:""}));
  }
  ...
  function pesoDiff(pesos,i) {
    if(i>=pesos.length-1) return null;
    const d=pesos[i].val-pesos[i+1].val;
    if(Math.abs(d)<0.05) return {v:"=",c:""};
    return d>0?{v:`+${d.toFixed(1)}kg`,c:"kup"}:{v:`${d.toFixed(1)}kg`,c:"kdown"};
  }

  const dietaAtual = selH?.dietas?.[0];
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="hdr">
          <div className="hrow">
            <div className="logo">
              <div className="lbox">TH9</div>
              <div><div className="ltxt">Consultoria TH9</div><div className="lsub">consultoriath9.com.br</div></div>
            </div>
            <div className="live"><div className="ldot"/>Ao vivo</div>
          </div>
          <div className="nav">
            {[{id:"dashboard",l:"Dashboard"},{id:"alunos",l:"Alunos"},{id:"cobrancas",l:"Cobranças"},{id:"projecao",l:"Projeção"}].map(t=>(
              <button key={t.id} className={`ntab ${nav===t.id?"active":""}`} onClick={()=>setNav(t.id)}>{t.l}</button>
            ))}
          </div>
        </div>
        ...
        <div className="content">
          ...
          {nav==="dashboard" && <>
            ...
            <div className="meta-card">
                ...
                <div>
                  ...
                  {!editMeta
                    ?<div style={{display:"flex",alignItems:"center",gap:8,marginTop:3}}>
                        <span style={{fontSize:13,fontWeight:700,color:"#ccc"}}>{fmtBRL(meta)}</span>
                        <button onClick={()=>setEditMeta(true)} style={{background:"none",border:"none",color:G,fontSize:10,cursor:"pointer",fontWeight:700,fontFamily:"Montserrat,sans-serif"}}>✏️</button>
                      </div>
                    :<div style={{display:"flex",alignItems:"center",gap:7,marginTop:3}}>
                        <input className="meta-inp" value={metaInput} onChange={e=>setMetaInput(e.target.value)} placeholder="Ex: 5000" type="number"/>
                        <button className="meta-btn" onClick={()=>{const v=parseFloat(metaInput);if(v>0){setMeta(v);setEditMeta(false);setMetaInput("");}}}>OK</button>
                        <button onClick={()=>setEditMeta(false)} style={{background:"none",border:"none",color:"#444",fontSize:11,cursor:"pointer",fontFamily:"Montserrat,sans-serif"}}>✕</button>
                      </div>
                  }
                </div>
              ...
              <div className="meta-vals">
                <span style={{fontSize:11,fontWeight:700,color:W}}>{fmtBRL(receita)} atingido</span>
                <span style={{fontSize:11,fontWeight:600,color:"#444"}}>faltam {fmtBRL(Math.max(0,meta-receita))}</span>
              </div>
            </div>
            ...
            {/* Gráfico */}
            <div className="sec">Faturamento Mensal</div>
            <div className="cc">
              <div className="cc-title">Evolução dos últimos 6 meses</div>
              <ResponsiveContainer width="100%" height={140}>
                <BarChart data={HIST_MENSAL} barSize={26}>
            ...
            <div className="sec">Crescimento de Alunos</div>
            <div className="cc">
              <div className="cc-title">Ativos por mês</div>
          ...
          {nav==="alunos" && <>
            ...
            {filtrados.length===0&&<div className="empty"><div className="ei">👥</div><div className="et">Nenhum aluno encontrado</div></div>}
            {filtrados.map(a=>{
              const dias=getDias(a.vencimento);
              const st=statusVenc(dias);
              const h=getH(a.id);
              const dietaOk=h.dietas?.length>0;
              const ultPeso=h.pesos?.[0];
              return (
                <div key={a.id} className="card">
                  <div className="chd">
                    <div><div className="cn">{a.nome}</div><div className="cs">{a.objetivo}</div></div>
          ...
          {nav==="cobrancas" && <>
            ...
            {vencidos.length>0&&<>
              <div className="alert-b"><div className="ad"/><div className="at">{vencidos.length} aluno{vencidos.length>1?"s":""} com mensalidade vencida</div></div>
              <div className="sec">Vencidos</div>
              {vencidos.map(a=>{
                const d=getDias(a.vencimento);
                return <div key={a.id} className="card" style={{borderColor:"#ef444422"}}>
            ...
            {proxVenc.length>0&&<>
              <div className="warn-b"><div className="wd"/><div className="wt">{proxVenc.length} vence{proxVenc.length>1?"m":""} em breve</div></div>
              <div className="sec">A Vencer (5 dias)</div>
              {proxVenc.map(a=>{
                const d=getDias(a.vencimento);
                return <div key={a.id} className="card" style={{borderColor:"#f9731622"}}>
            ...
            <div className="sec" style={{marginTop:16}}>Todos os Alunos</div>
            {alunos.map(a=>{
              const d=getDias(a.vencimento);
              const st=statusVenc(d);
          ...
          {nav==="projecao" && <>
            ...
            <div className="cc">
              {[1,2,3].map(n=>{
                const proj=receita+(ticketMedio*7*n);
                const d=new Date();d.setMonth(d.getMonth()+n);
                return <div key={n} className="rec-item">
                  <div><div style={{fontSize:12,fontWeight:700,color:W}}>{d.toLocaleString("pt-BR",{month:"short",year:"numeric"})}</div><div style={{fontSize:9,color:"#444",marginTop:2}}>+{7*n} alunos projetados</div></div>
                  <div style={{fontSize:13,fontWeight:800,color:G}}>{fmtBRL(proj)}</div>
                </div>;
              })}
            </div>
            ...
            <div className="cc">
              {[{a:30,l:"Próximo marco"},{a:50,l:"Escala média"},{a:100,l:"Escala premium"}].map(m=>{
                const fat=m.a*ticketMedio;
                const pct=Math.min(100,Math.round((ativos.length/m.a)*100));
                return <div key={m.a} style={{marginBottom:13}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                    <div><span style={{fontSize:11,fontWeight:700,color:W}}>{m.l}</span><span style={{fontSize:9,color:"#444",marginLeft:6}}>{m.a} alunos</span></div>
                    <span style={{fontSize:12,fontWeight:800,color:G}}>{fmtBRL(fat)}/mês</span>
                  </div>
        ...
        {modal==="cadastro"&&(
          <div className="overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
            <div className="modal">
              ...
              <div className="field"><div className="fl">Observações</div><textarea value={form.obs||""} onChange={e=>setForm(f=>({...f,obs:e.target.value}))} placeholder="Alergias, restrições, preferências..."/></div>
              <div className="brow">
        ...
        {modal==="historico"&&selected&&(
          <div className="overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
            <div className="modal">
              ...
              {/* DIETA */}
              {histTab==="dieta"&&<>
                <div className="sec">Nova Atualização de Dieta</div>
                ...
                <div className="field"><div className="fl">Refeições do dia</div><textarea value={dietaForm.refeicoes} onChange={e=>setDietaForm(f=>({...f,refeicoes:e.target.value}))} placeholder="Café: ovos + pão&#10;Almoço: frango + arroz + legumes&#10;Jantar: carne + batata-doce" style={{minHeight:90}}/></div>
                <div className="field"><div className="fl">Observações / Ajustes</div><textarea value={dietaForm.obs} onChange={e=>setDietaForm(f=>({...f,obs:e.target.value}))} placeholder="Ex: reduzimos 200kcal, foco em alimentos práticos..."/></div>
                <button className="btn btn-g" onClick={salvarDieta}>✓ Salvar Dieta</button>

                <div className="sec" style={{marginTop:16}}>Histórico de Dietas</div>
                {(selH?.dietas||[]).length===0&&<div style={{color:"#333",fontSize:11,textAlign:"center",padding:"12px 0"}}>Nenhuma dieta registrada</div>}
                {(selH?.dietas||[]).map((d,i)=>(
                  <div key={d.id} className={`hist-item ${expandHist===d.id?"active":""}`} onClick={()=>setExpandHist(expandHist===d.id?null:d.id)}>
                    ...
                    <div className="hi-prev">{d.kcal}kcal · {d.prot}g prot{d.carbo?` · ${d.carbo}g carbo`:""}{d.gord?` · ${d.gord}g gord`:""}</div>
                    {expandHist===d.id&&<>
                      {d.refeicoes&&<div className="hi-full">{d.refeicoes}</div>}
                      {d.obs&&<div style={{fontSize:10,color:"#666",marginTop:6,fontStyle:"italic"}}>{d.obs}</div>}
                      {i!==0&&<button className="btn btn-del btn-sm" style={{marginTop:8,width:"auto"}} onClick={e=>{e.stopPropagation();excluirHist("dietas",d.id)}}>🗑️ Excluir</button>}
                    </>}
                  </div>
                ))}
              </>}
              ...
              {histTab==="peso"&&<>
                ...
                <div className="field"><div className="fl">Observação</div><input value={novoHist.pesObs} onChange={e=>setNovoHist(n=>({...n,pesObs:e.target.value}))} placeholder="Ex: manhã em jejum"/></div>
                <button className="btn btn-g" onClick={salvarPeso}>✓ Registrar Peso</button>
                <div className="sec" style={{marginTop:16}}>Evolução de Peso</div>
                {(selH?.pesos||[]).length===0&&<div style={{color:"#333",fontSize:11,textAlign:"center",padding:"12px 0"}}>Nenhum peso registrado</div>}
                {(selH?.pesos||[]).map((p,i)=>{
                  const diff=pesoDiff(selH.pesos,i);
                  return <div key={p.id} className="peso-item">
                    <div><div className="peso-val">{p.val}kg</div><div className="peso-data">{fmtDate(p.data)}{p.obs&&` · ${p.obs}`}</div></div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      {diff&&<span style={{fontSize:10,fontWeight:700}} className={diff.c}>{diff.v}</span>}
                      {i===0&&<span style={{fontSize:8,color:G,fontWeight:700}}>ATUAL</span>}
                    </div>
                  </div>;
                })}
              </>}
              ...
              {histTab==="checkin"&&<>
                ...
                <div className="field"><div className="fl">Anotações</div><textarea value={novoHist.ciNota} onChange={e=>setNovoHist(n=>({...n,ciNota:e.target.value}))} placeholder="Treinos cumpridos, aderência à dieta, sono, feedback..." style={{minHeight:80}}/></div>
                <div className="field"><div className="fl">Peso registrado (opcional)</div><input value={novoHist.ciPeso} onChange={e=>setNovoHist(n=>({...n,ciPeso:e.target.value}))} placeholder="Ex: 82.5" type="number" step="0.1"/></div>
                <button className="btn btn-g" onClick={salvarCheckin}>✓ Salvar Check-in</button>
                <div className="sec" style={{marginTop:16}}>Check-ins Anteriores</div>
                {(selH?.checkins||[]).length===0&&<div style={{color:"#333",fontSize:11,textAlign:"center",padding:"12px 0"}}>Nenhum check-in registrado</div>}
                {(selH?.checkins||[]).map(c=>(
                  <div key={c.id} className="ci-card">
                    <div className="ci-top"><div className="ci-date">{fmtDate(c.data)}</div>{c.peso&&<span className="badge bg">⚖️ {c.peso}kg</span>}</div>
                    <div className="ci-nota">{c.nota}</div>
                  </div>
                ))}
              </>}
            </div>
        ...
        {wpModal&&(
          <div className="overlay" onClick={e=>e.target===e.currentTarget&&setWpModal(null)}>
            <div className="modal">
              <div className="mh"/>
              <div className="mt">💬 Mensagem de Cobrança</div>
              <div className="wp-prev">
                <div className="wp-lbl">Prévia — {wpModal.a.nome}</div>
                <div className="wp-txt">{gerarMsgCobranca(wpModal.a)}</div>
              </div>
